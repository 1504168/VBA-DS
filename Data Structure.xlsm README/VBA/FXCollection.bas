...
Public Function MakeValidConstName(ByVal GivenName As String) As String
    ...
    Dim Result As String
    Result = GivenName
    Result = RemoveStartingDigits(GetOnlyAlphanumericCharcter(Result, vbNullString, True, True))
...
Public Function RemoveStartingDigits(ByVal FromText As String) As String
    ...
    For Index = 1 To Len(FromText)
        ...
        If Not IsDigit(CurrentCharacter) Then
            Result = Mid(FromText, Index)
            Exit For
        End If
    Next Index

    RemoveStartingDigits = Result
...
Public Function ConvertRangeDataToArrayCode(ByVal FromRange As Range _
    ...
    For Each CurrentValue In FromRange
        ...
        CurrentValue = AddDoubleQuotesIfString(CurrentValue.Value)
...
Public Function ConvertRangeDataToCollectionCode(ByVal FromRange As Range _
                                                 , ByVal CollectionVarName As String) As String

    '@Description("This will generate Collection declaration Code. If two column then it will consider first col as key. See Example Call.")
    '@Dependency("No Dependency")
    '@ExampleCall : ConvertRangeDataToCollectionCode(Range("A1:A3"),"Properties")
    '  Output :
    '    Set Properties = New Collection
    '    With Properties
    '       .Add item := "RangeLabel",key := "RangeLabel"
    '       .Add item := "RangeReference",key := "RangeReference"
    '       .Add item := "Level",key := "Level"
    'Where A1:A3 has "RangeLabel,RangeReference,Level"
    '@Date : 30 Jan 2025 07:02:42 PM
    '@PossibleError:

    Dim FinalCode As String
    FinalCode = "    Dim " & CollectionVarName & " As Collection" & vbNewLine & _
                "    Set " & CollectionVarName & " = New Collection" & vbNewLine & _
                "    With " & CollectionVarName
    
    
    Dim CurrentValue As Variant
    Dim CurrentLineCode As String
    If FromRange.Columns.Count = 2 Then
        
        Dim RowIndex As Long
        For RowIndex = 1 To FromRange.Rows.CountLarge
            
            CurrentLineCode = "        .Add item :=" _
                              & AddDoubleQuotesIfString(FromRange.Cells(RowIndex, 2).Value) _
                              & " , key :=" & AddDoubleQuotesIfString(FromRange.Cells(RowIndex, 1).Value, True)
            FinalCode = FinalCode & vbNewLine & CurrentLineCode
    ...
    Else
        ...
        For Each CurrentValue In FromRange.Value
        
            CurrentLineCode = "        .Add item :=" _
                              & AddDoubleQuotesIfString(CurrentValue) _
                              & " , key :=" & AddDoubleQuotesIfString(CurrentValue, True)
    ...
    FinalCode = FinalCode & vbNewLine & "   End With"
    
    ConvertRangeDataToCollectionCode = FinalCode
    
End Function

Private Function AddDoubleQuotesIfString(ByVal Value As Variant, Optional ByVal IsMandatory As Boolean = False) As String
    
    Dim Result As String
    Const DOUBLE_QUOTES As String = """"
    If Not IsNumeric(Value) Or IsMandatory Then
        Result = DOUBLE_QUOTES & Replace(Value, DOUBLE_QUOTES, DOUBLE_QUOTES & DOUBLE_QUOTES) & DOUBLE_QUOTES
    Else
        Result = Value
    End If
    
    AddDoubleQuotesIfString = Result
...
Public Function GenerateRandomNumbersWithinRange(ByVal NumberOfRows As Long _
...
Private Function HStack(ByVal FirstArray As Variant, ByVal SecondArray As Variant) As Variant
    ...
    TotalRowCount = MaxValue(NumberOfRowIn2DArray(FirstArray), NumberOfRowIn2DArray(SecondArray))
...
Public Function MaxValue(ByVal X As Variant, ByVal Y As Variant) As Variant
...
Public Function GetTemporaryFolderPath() As String
    GetTemporaryFolderPath = Environ$("tmp")
End Function

Public Function GetVisibleCellsData(FromRange As Range) As Variant
    
    ' This will retrieve only visible cells data as a 2D array.
    
    
    Dim VisibleCells As Range
    Set VisibleCells = FromRange.SpecialCells(xlCellTypeVisible)
    ...
    Dim Result() As Variant
    ReDim Result(1 To TotalRowCount, 1 To VisibleCells.Areas(1).Columns.Count)
    
    Dim RowIndexInResult As Long
    RowIndexInResult = 1
    
    Dim CurrentAreaData As Variant
    For Each Area In VisibleCells.Areas
        
        CurrentAreaData = Area.Value
        Dim RowIndex As Long
        For RowIndex = LBound(CurrentAreaData, 1) To UBound(CurrentAreaData, 1)
            
            Dim ColIndex As Long
            For ColIndex = LBound(CurrentAreaData, 2) To UBound(CurrentAreaData, 2)
                Result(RowIndexInResult, ColIndex) = CurrentAreaData(RowIndex, ColIndex)
            Next ColIndex
            
            RowIndexInResult = RowIndexInResult + 1
            
        Next RowIndex
        
    Next Area
    
    GetVisibleCellsData = Result
    
End Function

Public Function SplitArrayByColumn(InputArray As Variant, ColumnIndex As Long, HasHeaders As Boolean) As Collection
    
    ' Split a 2D array by a column distinct item and return corresponding groups as Collection
    
    Dim KeysCollection As Collection
    Set KeysCollection = New Collection
    
    Dim Offset As Long
    Offset = IIf(HasHeaders, 1, 0)
    
    ' Store keys and row indexes
    Dim RowIndex As Long
    For RowIndex = LBound(InputArray, 1) + Offset To UBound(InputArray, 1)
        
        Dim CurrentKey As Variant
        CurrentKey = InputArray(RowIndex, ColumnIndex)
        
        If IsExistInCollection(KeysCollection, CurrentKey) Then
            Dim Temp As Collection
            Set Temp = KeysCollection.Item(CurrentKey)
            Temp.Add RowIndex
        Else
            Dim RowIndexColl As Collection
            Set RowIndexColl = New Collection
            If HasHeaders Then RowIndexColl.Add LBound(InputArray, 1)
            RowIndexColl.Add RowIndex
            KeysCollection.Add RowIndexColl, CurrentKey
        End If
    ...
    For KeyCounter = 1 To KeysCollection.Count
        Dim CurrentIndexes As Collection
        Set CurrentIndexes = KeysCollection(KeyCounter)
        
        Dim ResultArray() As Variant
        ReDim ResultArray(1 To CurrentIndexes.Count, LBound(InputArray, 2) To UBound(InputArray, 2))
        
        ' Copy data rows
        Dim ResultRowIndex As Long
        ResultRowIndex = 1
        Dim SourceRowIndex As Variant
        For Each SourceRowIndex In CurrentIndexes
            Dim ColIndex As Long
            For ColIndex = LBound(InputArray, 2) To UBound(InputArray, 2)
                ResultArray(ResultRowIndex, ColIndex) = InputArray(SourceRowIndex, ColIndex)
            Next ColIndex
    ...
    Set SplitArrayByColumn = ResultCollection
    
End Function

Public Function IsInsideArrayBound(ByVal ForArr As Variant _
                                   , Optional ByVal RowIndex As Long = -100 _
                                    , Optional ByVal ColIndex As Long = -100) As Boolean
    
    Dim CheckForRow As Boolean
    CheckForRow = True
    
    If RowIndex <> -100 Then
        CheckForRow = (RowIndex >= LBound(ForArr, 1) _
                  And RowIndex <= UBound(ForArr, 1))
                
    End If
    
    Dim CheckForCol As Boolean
    CheckForCol = True
    If ColIndex <> -100 Then
        CheckForCol = (ColIndex >= LBound(ForArr, 2) _
                  And ColIndex <= UBound(ForArr, 2))
    End If
    
    IsInsideArrayBound = (CheckForRow And CheckForCol)
